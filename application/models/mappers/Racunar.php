...
/**
 * Data Mapper implementation for Application_Model_Racunar
 *
 * @package Application_Model
 * @subpackage Mapper
 * @author SpecNaz Team 9815
 */
class Application_Model_Mapper_Racunar extends Application_Model_Mapper_MapperAbstract
{
    /**
     * Returns an array, keys are the field names.
     *
     * @param Application_Model_Racunar $model
     * @return array
     */
    public function toArray($model)
    {
        if (! $model instanceof Application_Model_Racunar) {
        ...
        $result = array(
            ...
            'id_tip' => $model->getIdTip(),
            'id_os' => $model->getIdOs(),
            'id_mb' => $model->getIdMb(),
            'id_cpu' => $model->getIdCpu(),
            'id_korisnik' => $model->getIdKorisnik(),
        );
     ...
     *
     * @return Application_Model_DbTable_Racunar
     */
    public function getDbTable()
    {
        if ($this->_dbTable === null) {
            $this->setDbTable('Application_Model_DbTable_Racunar');
        }
     ...
     *
     * @param Application_Model_Racunar $model The model to delete
     ...
     */
    public function save(Application_Model_Racunar $model,
     ...
     * @param Application_Model_Racunar|null $model
     * @return Application_Model_Racunar|null The object provided or null if not found
     */
     ...
     * @param Application_Model_Racunar|null $entry The object to load the data into, or null to have one created
     * @return Application_Model_Racunar The model with the data provided
     */
    public function loadModel($data, $entry)
    {
        if ($entry === null) {
            $entry = new Application_Model_Racunar();
        }

        if (is_array($data)) {
            $entry->setId($data['id'])
                  ->setNaziv($data['naziv'])
                  ->setIdTip($data['id_tip'])
                  ->setIdOs($data['id_os'])
                  ->setIdMb($data['id_mb'])
                  ->setIdCpu($data['id_cpu'])
                  ->setIdKorisnik($data['id_korisnik']);
