...
<?php if (strpos($column['type'], 'datetime') !== false): ?>
        if ($returnZendDate) {
            if ($this->_data['<?=$column['field']?>'] === null) {
                return null;
            }
...
$db = get_class($this);
if (stripos($db, 'mssql') !== false || stripos($db, 'dblib') !== false || stripos($db, 'sqlsrv') !== false): ?>
            return new Zend_Date($this->_data['<?=$column['field']?>'], 'YYYY-MM-ddTHH:mm:ss.S');
<?php else: ?>
            return new Zend_Date($this->_data['<?=$column['field']?>'], Zend_Date::ISO_8601);
<?php endif; ?>
        }

        return $this->_data['<?=$column['field']?>'];
<?php elseif ($column['phptype'] == 'boolean'): ?>
        return $this->_data['<?=$column['field']?>'] ? true : false;
<?php else: ?>
        return $this->_data['<?=$column['field']?>'];
<?php endif; ?>
    }
